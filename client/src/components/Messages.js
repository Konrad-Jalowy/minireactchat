function Messages({messages}){
    return (<ul className="messages-list">
        {messages.map((msg) => {
            ...
            return <li key={_time}><strong><span className="time-badge">{formattedTime}</span> <span className="user-badge">{msg.name}</span></strong> says: <span className="message">{msg.message}</span></li>
        })}
